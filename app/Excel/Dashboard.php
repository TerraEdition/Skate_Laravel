...
class Dashboard
{
    public static function export_excel_by_pass()
    {
        ...
        $sheet->mergeCells('A1:' . Excel::number_to_alphabet(($group->count() - 1) + 5) . '1');
        ...
        $sheet->setCellValueExplicit('B4', 'Tim', DataType::TYPE_STRING);
        $sheet->getStyle('B4')->applyFromArray(array_merge_recursive(Excel::center_middle()));
        $sheet->mergeCells('B4:B6');
        $sheet->setCellValueExplicit('C4', 'Kode Tim', DataType::TYPE_STRING);
        $sheet->getStyle('C4')->applyFromArray(array_merge_recursive(Excel::center_middle()));
        $sheet->mergeCells('C4:C6');
        $sheet->setCellValueExplicit('D4', 'No BIB', DataType::TYPE_STRING);
        $sheet->getStyle('D4')->applyFromArray(array_merge_recursive(Excel::center_middle()));
        $sheet->mergeCells('D4:D6');
        $sheet->setCellValueExplicit('E4', 'Group', DataType::TYPE_STRING);
        $sheet->getStyle('E4')->applyFromArray(array_merge_recursive(Excel::center_middle()));
        $sheet->mergeCells('E4:' . Excel::number_to_alphabet(($group->count() - 1) + 5) . '4');
        $col = "E";
        $row = 6;
        foreach ($group as $g) {
        ...
        $sheet->setCellValueExplicit('E5', 'isi dengan v untuk mendaftar', DataType::TYPE_STRING);
        $sheet->mergeCells('E5:' . Excel::number_to_alphabet(($group->count() - 1) + 5) . '5');
        $sheet->getStyle('E5')->applyFromArray(array_merge_recursive(Excel::center_middle()));
